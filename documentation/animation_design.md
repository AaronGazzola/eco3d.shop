...
- **Step 6 — rear hip + back legs**: code landed (commits `334a8bc` and `c82f6eb`) under OpenSpec change `add-rear-hip-back-legs`. Cascade extends through both hip-bearing spines; each hip runs an independent `runHipStep` with its own `HipState` + foot pair; back legs render via the same single-bone `applyLegBone` helper; four foot markers render in distinct colors; `FrameSnapshot` restructured to `frontHip`/`rearHip` nested form.
- **Step 6 — partial**: small attractor moves work correctly. Large turns surface a deeper architectural gap: the body bends in place and slides through world, feet shuffle along without stepping. See "Open issue" below.
- **Done**: Steps 1–5 (verified), Step 6 mechanical (legs render attached to body across all parent rotations).
...
- **What Step 6 adds in code**: `buildCascadeChain` extends to the second hip-bearing spine. `findRearHip` mirrors `findFrontHip`. `useLocomotion` restructured around `HipRuntime { state, feet, initId }` per hip, with `runHipStep` as the shared per-hip step decision loop and `applyHipLegs` as the shared rendering call. Front and rear hips run independently; within a hip only one foot swings at a time, but front and rear may swing simultaneously. `applyLegBone` reads `hipPivot.matrixWorld` (not just local quaternion) and rotates the leg in the pivot's world frame, so the leg renders attached to the body even when the front hip is translated in world by mid-spine rotations.
- **Open issue (the spec ≠ implementation gap surfaced during Step 6 verification)**: per design § 2, "planted feet are pinned to their current world position" and "rotating spine joints strains planted feet." The current implementation pins feet in the **owning hip's local frame** and computes strain only against that hip's own local cascade slot. In Step 5 these coincided (front hip was the deepest cascade member, no parent rotations). In Step 6 they diverge: when spine-3..5 rotate to satisfy a large desired head yaw, the front hip translates significantly in world (recorded: spine-2 worldPos.z drifts from 0 → 3.0 across a single turn), but the front foot's strain calculation sees the hip's *local* slot unchanged, so no step fires. The body bends, the front half slides through world, the feet ride along in the front hip's local frame ("shuffling"). The rear hip never engages because mid-spine joints consume all the leftover cascade demand before it reaches the rear. **The fix shape**: foot state becomes true world coordinates; `ensureHipInit` plants feet at `hipPivot.parent.matrixWorld.transformPoint(modelFoot − hipBack)`; `computeStrain` measures distance in world frame using `parent.matrixWorld.transformPoint(footTargetAt(localYaw))`. Once world-anchored, mid-spine rotations create real strain, feet step, and the rear hip starts seeing cascade demand because the spine joints are no longer free to fill greedily.
...
- **Pick up here**: world-anchored foot refactor (see "Open issue" above). Until that lands, large turns will continue to bend-and-slide rather than step. After the refactor, retest Step 6's "passes if" criteria including diagonal-couplet alternation. Then proceed to Step 7 (emergent walking — no new code) and Step 8 (unfreeze tail).
