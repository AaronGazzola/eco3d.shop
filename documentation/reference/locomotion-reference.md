...
**Our kinematic substitution:** we do not simulate torque or physics. We set the joint
angle directly from the same antagonist difference:
```
bend_angle_i  ∝  xᵢˡ − xᵢʳ  =  rᵢˡ·(1 + cos θᵢˡ) − rᵢʳ·(1 + cos θᵢʳ)
```
With the left/right pair held antiphase by the intrasegmental coupling (`φ = π`), this
difference is a clean signed oscillation centered on zero. The proportionality constant
is the joint's studio `angleCap`. This is a faithful reduction of the paper's active
term, not an invention — but it **is** the one place we depart, so it's called out here.
...
- Foot-ground contact in the paper emerges from physics. In our kinematic rig we will
  place the foot in the scene from the limb phase (stance = planted, swing = arc). That
  placement logic is **project glue, not in the paper.**
...
**We adapt (clearly bounded, outside the oscillator core):**
- Output → joint **angle** instead of output → muscle torque → physics (§4).
- Limb phase → **foot world position** in our 3D scene (§5).
- Attractor → **drive** (and a left/right drive split for steering, §6).
