...
export const usePageStore = create<PageState>()(persist((set) => {
  ...
  return {
    ...
    bodyConnectionParams: { position: 0.32, yaw: -0.1, pitch: -0.95, roll: 0 },
    setBodyConnectionParams: (params) => set((state) => ({ bodyConnectionParams: { ...state.bodyConnectionParams, ...params } })),
    bodyToBodyConnectionParams: { position: 0.37, yaw: 0, pitch: -0.95, roll: 0 },
    setBodyToBodyConnectionParams: (params) => set((state) => ({ bodyToBodyConnectionParams: { ...state.bodyToBodyConnectionParams, ...params } })),
    bodyToTailConnectionParams: { position: 0.33, yaw: 2.691592653589793, pitch: -0.9, roll: 0 },
    setBodyToTailConnectionParams: (params) => set((state) => ({ bodyToTailConnectionParams: { ...state.bodyToTailConnectionParams, ...params } })),
    headBodyLimits: { positionMin: 0.32, positionMax: 0.81, yawMin: -0.7, yawMax: 0.45, pitchMin: -1.25, pitchMax: -0.85, rollMin: -0.15, rollMax: 0.2 },
    setHeadBodyLimits: (limits) => set((state) => ({ headBodyLimits: { ...state.headBodyLimits, ...limits } })),
    bodyBodyLimits: { positionMin: 0.37, positionMax: 0.75, yawMin: -0.45, yawMax: 0.25, pitchMin: -1.25, pitchMax: -0.75, rollMin: -0.1, rollMax: 0.15 },
    setBodyBodyLimits: (limits) => set((state) => ({ bodyBodyLimits: { ...state.bodyBodyLimits, ...limits } })),
    bodyTailLimits: { positionMin: 0.33, positionMax: 0.73, yawMin: 2.691592653589793, yawMax: 3.241592653589793, pitchMin: -1, pitchMax: -0.4, rollMin: -0.15, rollMax: 0.2 },
    setBodyTailLimits: (limits) => set((state) => ({ bodyTailLimits: { ...state.bodyTailLimits, ...limits } })),
    bodyLinkCount: 1,
    addBodyLink: () => set((state) => ({ bodyLinkCount: state.bodyLinkCount + 1 })),
