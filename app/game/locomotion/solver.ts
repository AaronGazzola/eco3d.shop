import { BodySpec, SolverDiagnostics, SolverState } from './types'

export const FIXED_SUBSTEP_SECONDS = 0.002
export const MAX_FRAME_SECONDS = 0.05
export const JOINT_DAMPING = 0.6
export const LIMIT_STOP_STIFFNESS = 80
export const LIMIT_STOP_DAMPING = 3

function degreesOfFreedom(spec: BodySpec): number {
  return 3 + spec.joints.length
}
...
export function perturb(state: SolverState, spec: BodySpec, magnitude: number): void {
...
export function diagnostics(
  ...
  for (let j = 0; j < spec.joints.length; j++) {
    ...
    const angle = state.jointAngles[j]
    const cap = angle >= 0 ? joint.yawForwardLimit : joint.yawBackwardLimit
  ...
  return {
    ...
    maxJointAngleFractionOfCap: maxFraction,
  }
}
