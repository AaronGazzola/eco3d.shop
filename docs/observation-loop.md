...
> **What the loop is optimising for lives in `animation-roadmap.md` §6** (the three success metrics
> and the preset grid), and the stage order lives in §3, Phase D-T. Read both before picking a lever.
> The iteration protocol itself — one lever per step, a clickable config link every time, presets only
> on owner approval — is the `/observe-loop` skill. This document is the harness reference.
...
```bash
# 1. Server up with the CURRENT build. NEVER observe a dev server, and rebuild + restart after
#    ANY app code change — a running prod server keeps serving the old bundle.
npm run prod:3002   # build + serve; detach it so it survives across commands (see "Server")
...
> **Grip is retired** (roadmap Decision 10): a pinned foot is a constraint, and it flattened the axial
> wave into a standing wave. Foot **thrust** replaces it. The switches and timing params below still
> exist in `SimConfig` and still work, so this section stays accurate — but `gripEnabled` should stay
> off. The *timing* machinery is not retired and is exactly what thrust is clocked from: the window
> that used to open the grip now gates the backward push. `--events` remains the way to verify that
> timing against an undisturbed wave.
...
**lift** (swing) = `rel >= stepDuty`, where `rel = (phase − shift) mod 1`.

> **The front and hind girdles need SEPARATE shifts.** They sit half a cycle apart: at any given
> girdle-clock phase the front feet are at max-forward reach while the hind feet are at max-backward.
> Under MuJoCo the window therefore uses `footThrustShift` for the front pair and `footThrustShiftHind`
> for the hind pair — the same two values the thrust fires on, so the diagnostic and the actuation
> cannot disagree. Sharing one shift silently puts the hind windows on the **swing**, which reads the
> wrong half of the stroke and makes every hind number wrong. Always run the window-placement check:
> a correct window starts at the foot's most forward body-frame position and ends at its most backward.
