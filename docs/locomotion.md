...
> `docs/reference/locomotion-reference.md` — the verified extraction of the
> source paper. It is the single source of truth. If anything here restates a formula or
> number, the reference wins.

> **Current build state + the staged plan:** see `docs/animation-roadmap.md`
...
5. **Environment forces** — **hydrodynamics** (reactive + resistive) for water; **ground
   ...
   *Our deviation:* on land we replace the paper's contact friction with a **foot thrust**
   term (roadmap Decision 10). Their robot's feet bear load and grip; our printed model sits
   flush on the floor with frictionless contacts, so a foot has nothing to push against. The
   thrust term supplies that push directly, clocked off the same limb CPG phase.
...
- **Land propulsion → foot thrust, not foot grip** (roadmap **Decision 10**, supersedes the grip
  pin). A pinned foot is a hard constraint on the body; it fought the axial wave and flattened it.
  Thrust is an applied force, so the wave is mathematically untouched. Turning falls out of a
  left/right thrust imbalance rather than needing a heading, which the body does not have.
...
- **`docs/reference/locomotion-reference.md`** — verified model; single source of
  truth for all math and parameters.
- **`docs/reference/knusel-2020-salamander-cpg.pdf`** — the paper (Knüsel et al.
  2020, *Frontiers in Neurorobotics* 14:604426). Open access.
- **`docs/animation-roadmap.md`** — the living plan, decision log, and the
