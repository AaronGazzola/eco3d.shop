...
The first attempt at Phase A of the salamander-CPG rebuild (`docs/animation-roadmap.md` § 3) landed body-spec extraction, the planar multibody solver, force model, render wiring, and sidebar controls in a single commit. The combined change could not pass its own visual gate, because the render path silently dropped the body's root transform (the computed `rootX / rootZ / rootHeadingY` were written to a `rootRef` that no JSX `<group>` was bound to) and rendered leg groups as world-space siblings of the spine chain rather than children of their attached spine's pivot. Joint angles also clamped on render regardless of solver state, so a misbehaving solver looked identical to a well-behaved one.
