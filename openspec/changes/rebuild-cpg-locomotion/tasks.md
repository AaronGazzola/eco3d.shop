...
- [ ] 1.1 Implement the pure CPG core in `cpg.ts` from the reference: phase eq, first-order amplitude `ṙ = a(R−r)`, output `x = r(1+cos θ)`, `ν = d·e`, `R = d·P(d, d_th)`
- [ ] 1.2 Add a left/right oscillator pair on a single chosen spine joint, antiphase (`φ = π`), with a fixed test `drive`
- [ ] 1.3 Apply that joint's bend as a LOCAL rotation `k·(x_left − x_right)` clamped to `effectiveAngleCaps(group).yaw`
...
- [ ] 2.1 Extend `chain.ts` so the body chain spans head → spines → tail (remove the stop-at-last-hip behaviour)
- [ ] 2.2 Implement `buildCpgNetwork(groups)`: a left/right axial pair per chain group; rostrocaudal, caudorostral, and intrasegmental couplings using the reference Table 2 weights/biases
- [ ] 2.3 Tick all axial oscillators each frame; apply each group's local bend clamped to its cap
...
- [ ] 3.1 Wire the saturation drive mapping (`ν = d·e`, `R = d·P(d, d_th)`) so amplitude/frequency follow `drive`
- [ ] 3.2 Add a manual `drive` slider to `animateStore.ts` + `AnimateSidebar.tsx` for verification without an attractor
...
- [ ] 4.1 Add limb oscillators (one per hip socket with a leg) and limb-to-girdle couplings in `buildCpgNetwork`
- [ ] 4.2 Bake diagonal-trot initial phases (front-left/rear-right vs front-right/rear-left offset by π)
- [ ] 4.3 Rebuild `foot.ts`: stance/swing from `sin(θ)`, swing-arc with lift; reuse the existing easing
- [ ] 4.4 Render legs via the unchanged `applyLegBone` pointing at the phase-driven foot world position
...
- [ ] 5.1 Derive a stable body-forward from the root/tail segment's world frame (never the head)
- [ ] 5.2 Latch the next planted foot position along that stable body-forward on stance→swing; `stride` scales with drive
