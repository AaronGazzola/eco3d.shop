...
4. **Semi-implicit drag (energy-runaway fix).** Even at gain 1 a slow energy runaway remained: over 30–60 s the kinetic energy climbed without bound (KE 79,000 by 45 s) and the body floated up and tumbled. Bisection (per drag term, headless) isolated it to the **explicit application of the resistive force**: each drag force is dissipative at the instant it is sampled (F·v ≤ 0 always), but applying it as an external force *before* `world.step()` lets forward-Euler integration overshoot between steps and inject energy — worse the larger `C_n` is (`C_n=0.6` → KE 2.9e5; `C_n=0.1` → calm), so simply lowering `C_n` would have destroyed the ≥10:1 anisotropy thrust needs. Fix: apply the drag as **semi-implicit (exponential) velocity damping *after* the step** — split each body's velocity into along-/across-segment parts and scale by `exp(−C·L·dt/m)` (and angular by `exp(−C_ω·L·dt/I)`). Unconditionally stable, strictly dissipative at any coefficient, anisotropy preserved. Validated 60 s on the curved rig: KE bounded 30–440, `comY` pinned at 0 (no float), still swims head-first (Δx −9.5 to −16 over 30 s). `applyEnvironment3D(body, dt)` now runs post-step and writes `setLinvel`/`setAngvel`.

## Open Questions

- **Out-of-plane drift.** ~~Does the body stay roughly in its starting plane, or tumble in pitch/roll?~~ Largely resolved headless with the stable drag: over a 60 s curved-rig run `comY` stays pinned at 0.000 (no vertical float) and KE stays bounded — no tumble blow-up. Pure-yaw axial joints + symmetric drag keep it planar. Still worth a visual confirm at the browser gate (task 8.3).
...
- **Drag re-tune.** ~~The 3D perpendicular plane has two axes vs one in 2D; effective normal drag on the body may differ.~~ Resolved: the semi-implicit form is stable at any `C_n`, so the values `0.6/0.05/0.03` hold and `C_n` can even be raised to sharpen thrust without blowing up. Final swim speed/feel is the deferred visual tuning in AZ-33.
